...
################################################
## WORKDIR      - 이후 명령 실행 디렉토리 설정 (cd 개념)
## COPY         - 호스트에서 컨테이너 이미지로 파일/디렉토리 복사
## RUN          - 이미지 빌드 시 실행
## CMD          - 컨테이너 시작 시 기본 실행 (덮어쓸 수 있음)
## ENTRYPOINT   - 컨테이너 시작 시 무조건 실행 (CMD보다 우선)
################################################
# 빌드 명령어 !
#   1. 빌드
...
#   2. 실행 (로컬 포트 5173과 연결)
##     → docker run -p 5173:5173 <도커 이미지명>
